<template>
  ...
  >
    <Icon icon="icon-park-outline:dashboard-one" class="dashboard-icon" />
    <transition>
      <span v-show="!isCollapse" class="menu-item-words animate__animated animate__bounceIn ">{{$t('sider.dashboard.title')}}</span>
    </transition>
  </el-menu-item>
    <el-menu-item
        index="/upload"
        class="menu-item"
        @click="goTo('/upload')"
    >    <SvgIcon name="test-vue" size="20" :color="isActiveIcon('/upload')"></SvgIcon>
      <transition

      >
        <div class="menu-item-words animate__animated animate__bounceIn" v-show="!isCollapse">{{$t('sider.upload.title')}}</div>
      </transition >
    </el-menu-item>
  </el-menu>
